...
/*   Updated: 2023/08/10 05:29:32 by afalconi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	lx_second_token_ck(char token, char *old_token)
{
	...
	{
		if (*old_token == CMD || *old_token == ARG
			|| *old_token == CL_S || *old_token == INP
			|| *old_token == HDOC || *old_token == APP || *old_token == OUT)
			return (-1);
	...
	{
		if (*old_token == CMD || *old_token == OP_S || *old_token == ARG || *old_token == CL_S)
			return (-1);
		return (0);
	}
	return (1);
...
static int	lx_all_token_ck(char token)
{
	...
	if (!old_token)
	{
		if (token != CMD && token != OP_S && token
			&& token != INP && token != OUT && token != HDOC)
			return (-1);
		old_token = token;
		return (0);
	}
	...
	{
		if (!(old_token == CMD || old_token == OP_S || old_token == CL_S
				|| old_token == ARG || old_token == INP || old_token == OUT
				|| old_token == APP || old_token == HDOC))
			return (-1);
		old_token = token;
		return (0);
	}
